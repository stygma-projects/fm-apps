...
export const useUser = () => {
  ...
  const getById = (id: string) =>
    useTrpcQuery('user', () => $trpc.auth.user.getById.query(id))

  const signUpByMailAndPassword = useTrpcMutation(
    (input: SignUpByMailAndPasswordInput) =>
      $trpc.auth.user.signUpByMailAndPassword.mutate(input),
  )

  const signInByMailAndPassword = useTrpcMutation(
    (input: SignInByMailAndPasswordInput) =>
      $trpc.auth.user.signInByMailAndPassword.mutate(input),
  )
