<template>
  <div
    class="grid grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-6 lg:gap-x-6 lg:gap-y-8"
  >
    <PrimeCard
    ...
    >
      ...
      <template #header>
        <div
          class="relative w-full overflow-hidden rounded-t-lg h-36 lg:h-54 bg-gray-50"
        >
          <PrimeImage
            v-if="hasImage(item)"
            :src="item.imageUrl"
      ...
      <!-- Content Area -->
      <template #title>
        <slot name="card-label" :item="item" :selected="isSelected(item)">
          <div class="w-full p-5 flex items-center min-h-[20]">
            ...
            <div v-else class="w-full text-center">
              <p
                class="text-xs font-bold break-words sm:text-sm lg:text-base line-clamp-2"
              >
                {{ item.label }}
              </p>
            </div>
