...
describe('Product categories page', () => {
  ...
  const navigateToCategory = () => {
    cy.get('[data-cy="fmc-product-categories-card"]').contains('Boisson').click();
  ...
  const checkNumberOfCategoryInPage = () => {
    cy.get('@getProductCategories').then((interception: any) => {
      ...
      cy.get('[data-cy="fmc-product-categories-card"]').should('have.length', res);
    });
  };
  ...
  context('Desktop', () => {
    ...
    it('navigating to the products page by clicking on a category card', navigateToCategory)
